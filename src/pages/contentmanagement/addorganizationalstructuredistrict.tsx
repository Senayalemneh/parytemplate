...
import {
  createNewsorgStructureDistrict,
  getorgStructureDistrict,
  getorgStructureByIdDistrict,
  deleteorgStructureDistrict,
  updatorgStructureDistrict,
  uploadFile,
...
const OrganizationalStructure = () => {
  ...
  const getValidParents = async (currentId?: number): Promise<SelectItem[]> => {
    try {
      const response = await getorgStructureDistrict();
  ...
  const handleSubmit = async (values: typeof form.values) => {
    try {
      ...
      if (editingId) {
        await updatorgStructureDistrict({ id: editingId, ...payload });
        showNotification(
          t("orgstructureadmin.notifications.success.update"),
          "success"
        );
      } else {
        await createNewsorgStructureDistrict(payload);
        showNotification(
  ...
  const handleEdit = async (id: number) => {
    try {
      setLoading(true);
      const item = await getorgStructureByIdDistrict(id);
      if (!item) {
        showNotification(
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      await deleteorgStructureDistrict(id);
      showNotification(
