...
import {
  createNewsorgStructureRegional,
  getorgStructureRegional,
  getorgStructureById,
  deleteorgStructureRegional,
  updatorgStructureRegional,
  uploadFile,
...
const OrganizationalStructure = () => {
  ...
  const getValidParents = async (currentId?: number): Promise<SelectItem[]> => {
    try {
      const response = await getorgStructureRegional();
  ...
  const handleSubmit = async (values: typeof form.values) => {
    try {
      ...
      if (editingId) {
        await updatorgStructureRegional({ id: editingId, ...payload });
        showNotification(
          t("orgstructureadmin.notifications.success.update"),
          "success"
        );
      } else {
        await createNewsorgStructureRegional(payload);
        showNotification(
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      await deleteorgStructureRegional(id);
      showNotification(
